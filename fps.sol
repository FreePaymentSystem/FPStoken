 ...
 * КОД СЫРОЙ, ТЕСТИРОВАТЬ НЕОБХОДИМО НА testnet.
 */
contract SafeMath {
...
contract FreePaySystem is StandardToken, SafeMath {
    ...
    function FreePaySystem(address _icoContract) {
...
contract FreePaySystem is SafeMath {
    ...
    // 500.000 FPS tokens
    uint public constant supplyLimit = 500000000000000000000000;

    //  200.000 CTX is token for bountyOwner
    uint public constant bountyOwnersTokens = 200000000000000000000000;

    // 1 ETH = 2139 FPS
    uint public constant PRICE = 2139;
    ...
    function FreePaySystem(address _icoManager, address _preIcoToken) {
        assert(_preIcoToken != 0x0);
        assert(_icoManager != 0x0);

        freepaysystemToken = new FreePaySystem(this);
    ...
    function importTokens(address _account) {
        ...
        if (preIcoBalance > 0) {
            freepaysystemToken.emitTokens(_account, preIcoBalance);
    ...
    function buyTokens(address _buyer) private {
        ...
        freepaysystemToken.emitTokens(_buyer, tokensToEmit);
    ...
    function burnTokens(address _from, uint _value) onlyManager notMigrated {
        freepaysystemToken.burnTokens(_from, _value);
    }
